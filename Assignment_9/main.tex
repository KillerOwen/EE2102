...
\begin{enumerate}
\item Write functions to generate two normal distributions $Z_1$ and $Z_2$ using box muller method.
\item Assume values of constants,
\begin{align}
\sigma_X = 0.5\\
\sigma_Y = 0.8\\
\rho = 0.5\\
...
\mu_Y = 1.5
\end{align}
\item Run the functions to generate $Z_1$ and $Z_2$ 10000 times, and store all the outputs in a temporary array of size 2x10000.
\item Make three 2-D arrays for Matrices and Vectors using the above assumed values.
...
\item Mulitply the A matrix with $\vec{z}$ and add to $\bm{\mu}$.
