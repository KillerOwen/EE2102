...
BB' &= 0\\
B + B' &= 1\\
\implies \pr{A} &= \pr{A\brak{B+B'}}\\
		&= \pr{AB + AB'}
\end{align}
By using inclusion-exclusion principle,
\begin{align}
\pr{A} &= \pr{AB} + \pr{AB'} - \pr{\brak{AB}\brak{AB'}}\\
	&= \pr{AB} + \pr{AB'} - \pr{\brak{AA}\brak{BB'}}\\
	&= \pr{AB} + \pr{AB'}\label{eight}
\end{align}
We know that the value of probability ranges from 0 to 1.
\begin{align}
0 \leq \pr{AB'} \leq 1\\
\text{Adding \pr{AB} both sides}\notag\\
\pr{AB} \leq \pr{AB} + \pr{AB'}\\
\text{Substituting value from equation \brak{\ref{eight}}}\notag\\
